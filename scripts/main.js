...
function isInPhilippines(feature) {
  ...
  // Safe overlay setter
  function safeSetOverlay(windyAPI, overlay) {
    try {
      windyAPI.store.set('overlay', overlay);
    } catch (e) {
      console.warn('Overlay not available:', overlay, e);
    }
  }  
...
function bootWindyAndStart() {
  ...
  window.windyInit(options, async function (windyAPI) {
    try {
      const map = windyAPI.map; // Leaflet map from Windy

      // Try switching to rain after init, but tolerate plan limits
      safeSetOverlay(windyAPI, 'rain');

      // Load layers now that map exists
      await loadFaultLines(map);

      // Earthquakes: ensure your renderer accepts map
      if (typeof renderEarthquakes === 'function') {
        await renderEarthquakes(map);
      }
    } catch (err) {
      console.error('Initialization error after Windy loaded:', err);
      showCustomAlert('Initialization error after Windy loaded.');
    }
  });
}
...
// Example stub; ensure your real function accepts the map
async function renderEarthquakes(map) {
